...
func (e *UserEventHandler) HandleUserCreated(ctx context.Context, msg *msgp.EventMessage) error {
	...
	// Publish user created event if event service is available
	if e.userEventService != nil {
		userID, _ := userData["user_id"].(string)
		if userID == "" {
			userID = email // fallback to email as ID
		}
		if err := e.userEventService.PublishUserCreatedEvent(ctx, userID, email, username, "system"); err != nil {
			e.log.Errorf("Failed to publish user created event: %v", err)
		}
	}
