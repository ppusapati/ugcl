...
func PermissionModelToSQLC(m *models.Permission) sqlc.Permission {
	return sqlc.Permission{
		...
		Action:    m.Action,
		Subject:   m.Subject,
		Effect:    ModelToDBEffect(m.Effect),
...
func PermissionModelToGrantSQLC(m *models.Permission) sqlc.GrantPermissionParams {
	return sqlc.GrantPermissionParams{
		...
		Action:    m.Action,
		Subject:   m.Subject,
		Effect:    sqlc.PermissionEffect(m.Effect),
		// TenantID:  sql.NullString{String: m.TenantID, Valid: m.TenantID != ""},
	}
}
...
func PermissionSQLCToModel(row sqlc.Permission) *models.Permission {
	return &models.Permission{
		...
		Action:    row.Action,
		Subject:   row.Subject,
		Effect:    DBToModelEffect(row.Effect),
	}
}
...
func DBToModelEffect(e sqlc.PermissionEffect) models.Effect {
	switch e {
	case sqlc.PermissionEffectGrant:
		return models.EffectGrant
	case sqlc.PermissionEffectForbidden:
...
func ModelToDBEffect(e models.Effect) sqlc.PermissionEffect {
	switch e {
	case models.EffectGrant:
		return sqlc.PermissionEffectGrant
	case models.EffectForbidden:
		return sqlc.PermissionEffectForbidden
	default:
		return sqlc.PermissionEffectUnknown
	}
}
